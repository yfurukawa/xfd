...
TEST_F (NetworkDAOTest, readData) {
...
TEST_F (NetworkDAOTest, catchHostNotFoundExceptionWhenStablishConnection) {
    EXPECT_THROW(sut->openInputter("unknownHost:8080"), NetworkException);
    sut->closeInputter();
}

TEST_F (NetworkDAOTest, catchExceptionWhenStablishConnection) {
    EXPECT_THROW(sut->openInputter("192.168.1.1:8080"), NetworkException);
    sut->closeInputter();
}
